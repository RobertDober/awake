defmodule Awake.Parser.Grammar do
  @moduledoc ~S"""
  A Minipeg Grammar defining patterns
  """
  use Awake.Types

  # import Minipeg.{Combinators, Mappers, Parsers}
  import Minipeg.{Combinators, Parsers}
  import Minipeg.Parsers.LispyParser, only: [customized_s_exp_parser: 2]
  @moduledoc false
  ...
  defp field_name_parser do
    Awake.Fields.defined_field_names
    |> Enum.map(&field_lookup_parser/1)
    |> select()
  end

  @spec field_lookup_parser(binary()) :: t()
